...
for (var project in projects) {
	$('#projects').append('<div class="education-entry'+' '+projects[project].title.replace(/ /g)+'">'+'<a href="#">'+projects[project].title+'</a>'+'<div class="date-text">'+projects[project].dates+'</div>'+'<p><br>'+projects[project].description+'</p>'+'</div>'+
			'<br/>');
	for (var i = 0; i < projects[project].images.length; i++) {
			console.log(projects[project].title+' '+projects[project].images[i]);
			$( '#projects .education-entry.'+projects[project].title.replace(/ /g) ).append('<img src="'+projects[project].images[i]+'"/>');
	}
}
